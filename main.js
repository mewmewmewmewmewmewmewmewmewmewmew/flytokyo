...
import { TrainSystem } from './train.js?v=12.47';
import { FISH_U, fishUniforms, FISH_PROJ_GLSL, FISH_FRAG_GLSL, TOON_GLSL } from './fisheye.js?v=12.47';
import { CHARACTERS, DEFAULT_CHARACTER } from './characters.js?v=12.47';
...
function initScene(collision) {
  ...
  window.addEventListener('keydown', e => {
    ...
    if (e.code === 'KeyM' && mapsLinkEl) { e.preventDefault(); window.open(mapsLinkEl.href, '_blank', 'noopener,noreferrer'); }
  });
  ...
  (function animate() {
    ...
    if (mapsLinkEl) {
      ...
      // log₂ scale × 1.5: y≈4m → zoom 20; y≈500m → zoom 17; y≈2000m → zoom 14.
      const _mz = Math.round(Math.max(8, Math.min(20, (20 - Math.log2(Math.max(1, controls.birdPos.y))) * 1.5)));
      mapsLinkEl.href = `https://www.google.com/maps/@${_ml.toFixed(6)},${_mo.toFixed(6)},${_mz}z/data=!3m1!1e3`;
    }
